	...
	for (int i = 0; i < 11; i++)
       {
        ...
        long double delta_t, final_time;//каждые сколько секунд производить расчет и сколько всего времени рассчитвыать в секундах
        delta_t = 0.04167*24;
	final_time = delta_t*158 + JED;
        std::vector<std::vector<double*>> solutions;//вектор, хранящий положения всех планет в каждый момент времени
	std::vector<double> times;//вектор, содержащий значения времени, прошедшего с начала отсчета к рассматориваемому моменту
...
//		T += delta_t;
		...
		planets_cond_update(planets_cond_0, JED, de405);
		times.push_back(JED);
		solutions.push_back(planets_cond_0);
	}
	...
	{
		std::cout << i << " " <<  solutions[i][2][0] << " " << solutions[i][2][1] << " " << solutions[i][2][2] << std::endl;
	}
